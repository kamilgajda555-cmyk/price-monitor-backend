...
SQL_STATEMENTS = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_source_daily_stats_source_date ON source_daily_stats(source_id, stat_date DESC)",
    "CREATE INDEX IF NOT EXISTS idx_price_history_product_date ON price_history(product_id, checked_at DESC)",
    "CREATE INDEX IF NOT EXISTS idx_price_history_lookup ON price_history(product_id, source_id, checked_at DESC)",
...
def init_database():
    ...
    try:
        ...
        # Don't fail the whole startup - tables might already exist
        return False
